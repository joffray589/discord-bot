...
var DiscordBot = /** @class */ (function (_super) {
    ...
    DiscordBot.prototype.listen = function () {
        ...
        this._client.on("message", function (message) {
            _this.emit("received-message", message);
            if (!_this.ignoreMessage(message)) {
                _this.emit("parsing-message", message);
                ...
                _this._guildContextManager
                    ...
                    if (!message.author.bot && message.content.startsWith(commandPrefix)) {
                        ...
                        if (keyword && keyword !== "") {
                            ...
                            var command_1 = _this.getCommand(keyword);
                            if (command_1) {
                                _this.guildContextManager.getCommandSetting(message.guild.id, keyword)
                                    .then(function (settings) {
                                    var context = {
                                        bot: _this,
                                        botCommand: command_1,
                                        message: message,
                                        guildContext: guildContext,
                                        settings: settings
                                    };
                                    _this.emit("execute-command", context, command_1);
                                    command_1.execute(context);
                                })
                                    .catch(function (error) {
                                    _this.error("message event : getCommandSetting error : " + error);
                                });
                            }
                        }
                    }
                    ...
                    .catch(function (error) {
                    _this.error("message event : loadGuildContext error : " + error);
                });
            }
        });
        ...
        this._client.on("messageReactionRemove", function (reaction, user) {
            _this._guildContextManager.loadGuildContext(reaction.message.guild.id)
                ...
                .catch(function (error) {
                _this.error("messageReactionRemove event : loadGuildContext error : " + error);
            });
        });
        this._client.on("messageReactionRemoveAll", function (message) {
            _this._guildContextManager.loadGuildContext(message.guild.id)
                ...
                .catch(function (error) {
                _this.error("messageReactionRemoveAll event : loadGuildContext error : " + error);
            });
        });
        this._client.on("guildMemberRemove", function (member) {
            if (member.user.id === _this._client.user.id) {
                // The bot has been kicked :( , we can delete his settings
                _this.guildContextManager
                    .removeGuildContext(member.guild.id)
                    .catch(function (error) {
                    _this.error("guildMemberRemove event : removeGuildContext error : " + error);
                });
            }
        });
    };
